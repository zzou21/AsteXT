'''This file cleans and processes raw story txt files throuhg tokenization, removing punctuations, expanding contractions, removing stopwords, and other methods. After a story selected by the user has been cleaned, the story is stored in a JSON file, with the story name as the key.
'''
...
dataFile = "StoryContentCleaned.json"
...
class StoryProcessor:
    def __init__ (self):
        ...
        self.stopWords = set(stopwords.words("english")) #finds stopwords in the English language
    ...
    def process(self, storyContent):
        ...
        for sentence in storySentences:
            ...
            lemmatizedAndStopwordRemoved = " ".join([self.lemmatizer.lemmatize(word) for word in individualWords if word not in self.stopWords]) #first remove words that are stop words, then lemmatize the remaining words back to their original forms, such as "drove" to "drive"
            processedStory.append(lemmatizedAndStopwordRemoved)
        return " ".join(processedStory)

def fileSelecterForContraction():
    textProcessor = StoryProcessor()
